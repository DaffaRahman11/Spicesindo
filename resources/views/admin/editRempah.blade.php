<x-layout-admin>
    ...
    <h1 class="h3 ml-2 mb-4 text-gray-800">Edit Data Rempah</h1>
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary mb-3">Tabel Produk Rempah</h6>
            <div class="col-lg-8">
                <form class="user" action="/dashboard/data-rempah/{{ $rempah->slug }}" method="post">
                    @method('put')
                    @csrf
                    <div class="form-group row">
                        <div class="col-sm-6 mb-3 mb-sm-0">
                            <input type="text" class="form-control form-control-user @error('detail') is-invalid @enderror" id="namaRempah"
                                placeholder="Nama Rempah - Rempah"  name="namaRempah" value="{{ old('namaRempah',  $rempah -> namaRempah )}}">
                                @error('namaRempah')
                        ...
                        <div class="col-sm-6">
                            <input type="text" class="form-control form-control-user @error('slug') is-invalid @enderror" id="slug"  name="slug"
                                placeholder="Slug" value="{{ old('namaRempah',  $rempah -> slug )}}">
                                @error('slug')
                                <div class="invalid-feedback">{{$message}}
                                  </div>
                                @enderror
                    ...
                    <div class="form-group">
                        <div class="form-group">
                            ...
                            placeholder="Detail Rempah" value="{{ old('namaRempah',  $rempah -> detailRempah )}}">
                        @error('detailRempah')
                        <div class="invalid-feedback">{{$message}}
                          </div>
                        @enderror
